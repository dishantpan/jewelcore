...
public class MetalService {
    ...
    public MetalResponse updateMetal(
            Long id,
            MetalUpdateRequest request) {
        ...
        metalRepository.findByCode(request.code())
                .filter(existingMetal ->
                        !existingMetal.getId().equals(id))
                .ifPresent(existingMetal -> {
                    throw new IllegalArgumentException(
                            "Metal code already exists: "
                                    + request.code()
                    );
                });
